...
public class ZombieBoardRenderer
{
   ...
   *
   * @param main   The Main method that starts the program.
   * @param scenes The different screens visible in the program, such
   *               as the main menu.
   */
   ...
   *
   * @param primaryStage The primary game stage that the content is being placed into.
   * @return The scene object that contains the rendered 2D board.
   */
  public Scene zombieHouse2d(Stage primaryStage)
  {
    ...
    entityManager.createZombies(gameBoard, boardHeight, boardWidth, true);
    ...
    for (int col = 0; col < boardHeight; col++)
    {
      for (int row = 0; row < boardWidth; row++)
      {
        ...
        if (zombieCounter < numZombies)
        {
          ...
          if (entityManager.zombies.get(zombieCounter).zombieCircle != null)
          {
            root.getChildren().add(entityManager.zombies.get(zombieCounter).zombieCircle);
            zombieCounter++;
          }
        }
      }
  ...
  private class MainGameLoop extends AnimationTimer
  {
    public void handle(long now)
    {
      ...
      if (initZombieMovement)
      {
        for (int i = 0; i < entityManager.zombies.size(); i++)
        {
          entityManager.zombies.get(i).initiateZombieToRandomAngle();
