...
public class ProceduralMap
{
   ...
   * calculations for the art rooms and hallways simpler
   *
  ...
  private static void generatePaths(ArrayList<ArrayList<Rectangle>> regions)
  {
    ...
    for (int i = 1; i < 5; i++)
    {
      ...
      {
        if (r.region == i)
        { //i being the current region 1-4
          frontier.add(r);
          seed.paths.add(r);
          r.paths.add(seed);
        }
  ...
  {
    for (ArrayList<Rectangle> region : regions)
    {
      for (Rectangle r : region)
      {
        r.x *= 4;
  ...
  private static void fillMap(Tile[][] map, ArrayList<ArrayList<Rectangle>> regions, int difficulty)
  {
    ...
    for (Rectangle r : regions.get(4))
    {
      ...
      if (r.y + r.height == map.length - 1)
      { //down
        downExitCandidates.add(r);
      }
      if (r.x + r.width == map[0].length - 1)
      { //right
  ...
  {
    for (Rectangle[] aLevel : level)
    {
      for (int j = 0; j < level[0].length; j++)
      {
        System.out.print(aLevel[j].region);
      }
      System.out.print("\n");
    }
    System.out.println("--------------------------");
